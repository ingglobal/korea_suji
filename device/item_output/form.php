...
if ($member['mb_level'] < 3)
    goto_url(G5_BBS_URL."/login.php?url=".urlencode(G5_URL."/device/item_output/form.php"));
...
$sql = " SELECT *
                ...
                , ( SELECT SUM(itm_weight) FROM {$g5['item_table']} WHERE oop_idx = oop.oop_idx AND itm_status = 'finish' ) AS itm_total
                , ( SELECT COUNT(*) FROM {$g5['item_table']} WHERE oop_idx = oop.oop_idx AND itm_status = 'finish' ) AS itm_cnt
...
'bom_idx' : 완제품의 bom_idx => g5_1_order_out_practice 테이블의 bom_idx
'oop_idx' : g5_1_order_out_practice 테이블의 oop_idx
'bom_part_no' : g5_1_bom 테이블의 반제품의 bom_part_no
'itm_name' : g5_1_bom 테이블의 반제품의 bom_name
'itm_barcode' : 반제품 barcode는 가공해서 만들어야하는 값
...
<div id="tbl_box">
        ...
        <table>
            ...
            <thead>
                <tr>
                    ...
                    <th scope="col">반제품<br>재고량<br><span>[itm_total]</span></th>
                    <th scope="col">반제품<br>개수<br><span>[itm_cnt]</span></th>
                    <th scope="col">완제품<br>재고량<br><span>[itm_total]</span></th>
                    <th scope="col">완제품<br>개수<br><span>[itm_cnt]</span></th>
                    <th scope="col">측정량<br><span>(itm_weight)</span></th>
                    <th scope="col">출력</th>
                    <th scope="col">상세</th>
                </tr>
            </thead>
            ...
            <tr class="<?php echo $bg.$tr_focus; ?>" orp_idx="<?php echo $row['orp_idx'] ?>" bom_idx="<?=$row['bom_idx']?>">
                ...
                <td class="td_itm_cnt"><?=number_format($row['itm_cnt'])?></td>
                <td class="td_itm_weight"><input type="text" name="itm_weight" class="weight frm_input" value="" onclick="javascript:chk_number(this)"></td>
                <td class="td_itm_output">
    ...
    <div class="tbl_head02 tbl_wrap">
        ...
        <caption>해당제품의 상세재고목록</caption>
            <thead>
                <tr>
                    <th scope="col">완제ID<br><span>itm_idx</span></th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="t_itm_lot"><?=$row['itm_lot']?></td>
...
$('.btn_output').on('click',function(){
    ...
    var ipt = $(this).parent().siblings('.td_itm_weight').find('.weight').val();
    if(!ipt){
...
function form_output(obj,ipt,token){
    ...
    var cnt = Number(obj.attr('itm_cnt')) + 1;
    cnt = pad(cnt,3);
    var ab = 'A';
    var wt = ipt;
