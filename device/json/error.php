...
//	$list = array('');
    ...
    $dta_value_type = ($_REQUEST['dta_value_type']) ?: 'count';
    ...
    if(!$en_date) {
        $sql = "SELECT * FROM {$g5['data_error_table']} {$sql_search}
    ...
    $sql = "SELECT dta_dt, FROM_UNIXTIME(dta_dt,'%Y-%m-%d %H:%i:%s') AS dta_ymdhis
                    ...
                    , ".strtoupper($dta_value_type)."(dta_idx) AS dta_value
...
//    exit;
	...
	for($i=0;$row=sql_fetch_array($rs);$i++){
        ...
        $dta1[$val1]['y'] = (int)$row['dta_value'];
        $dta1[$val1]['dta_shf_no'] = (int)$row['dta_shf_no'];
        $dta1[$val1]['dta_code'] = $row['dta_code'];
        $dta1[$val1]['dta_message'] = $row['dta_message'];
    }
