...
// 디폴트(카운터그룹핑): http://icmms.co.kr/device/json/output.php?token=1099de5drf09&mms_idx=1
// http://icmms.co.kr/device/json/output.php?token=1099de5drf09&mms_idx=1&shf_no=1&dta_mmi_no=1&dta_item=minute&dta_unit=10&&st_date=2020-05-01&st_time=10:00:00&end_date=2020-05-31&end_time=23:59:59
...
//	$list = array('');
    ...
    // $where[] = " mms_idx = '".$mms_idx."' AND dta_group = '".$dta_group."' AND dta_defect IN (".implode(",",$dta_defects).") AND dta_status = 0 ";   // 디폴트 검색조건
    ...
    // dta_defect_type 불량타입 조건 (isset이 아님, 0으로 들어오면 조건을 타면 안 됨)
    if($_REQUEST['dta_defect_type'])
    ...
    if(!$en_date) {
        $sql = "SELECT * FROM {$g5['data_output_table']} {$sql_search}
    ...
    $sql = "SELECT dta_dt, FROM_UNIXTIME(dta_dt,'%Y-%m-%d %H:%i:%s') AS dta_ymdhis
            ...
            FROM {$g5['data_output_table']} {$sql_search}
                AND dta_dt >= '".$idx1."'
                AND dta_dt <= '".$idx2."'
...
//    exit;
	...
	for($i=0;$row=sql_fetch_array($rs);$i++){
        ...
        $dta1[$val1]['dta_defect'] = (int)$row['dta_defect'];
        $dta1[$val1]['dta_defect_type'] = (int)$row['dta_defect_type'];
        $dta1[$val1]['dta_message'] = (int)$row['dta_message'];
    }
    ...
    for($i=$idx1;$i<$idx2;$i+=$byunit) {
        ...
        $dta1[$val1]['yraw'] = $dta1[$val1]['y'];
        $dta1[$val1]['yamp'] = 1;
        $dta1[$val1]['ymove'] = 0;
        $dta1[$val1]['dta_date'] = date("Y-m-d H:i:s",$i);
        $dta1[$val1]['dta_shf_no'] = $dta1[$val1]['dta_shf_no'];
        $dta1[$val1]['dta_mmi_no'] = $dta1[$val1]['dta_mmi_no'];
        $dta1[$val1]['dta_defect'] = $dta1[$val1]['dta_defect'];
        $dta1[$val1]['dta_defect_type'] = $dta1[$val1]['dta_defect_type'];
        $dta1[$val1]['dta_message'] = $dta1[$val1]['dta_message'];
        $dta1[$val1]['cnt'] = $cnt;
        // 좌표값
        $list[$cnt] = $dta1[$val1];
        $cnt++;
