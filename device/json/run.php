...
// 디폴트(카운터그룹핑): http://icmms.co.kr/device/json/run.php?token=1099de5drf09&mms_idx=7
// http://icmms.co.kr/device/json/run.php?token=1099de5drf09&mms_idx=7&dta_group=run&shf_no=1&dta_mmi_no=1&dta_item=minute&dta_unit=10&&st_date=2020-04-29&st_time=10:00:00&en_date=2020-04-30&en_time=23:59:59
...
//	$list = array('');
    ...
    // 기본 설정
    $dta_group = ($_REQUEST['dta_group']) ?: 'run';
    ...
    if(!$en_date) {
        $sql = "SELECT * FROM {$g5['data_run_table']} {$sql_search}
    ...
    $sql = "SELECT dta_dt, FROM_UNIXTIME(dta_dt,'%Y-%m-%d %H:%i:%s') AS dta_ymdhis
            ...
            FROM {$g5['data_run_table']} {$sql_search}
                AND dta_dt >= '".$idx1."'
                AND dta_dt <= '".$idx2."'
	...
	for($i=0;$row=sql_fetch_array($rs);$i++){
        ...
        $dta1[$val1]['y'] = round($row['dta_value']/60,2);
