...
// 분, 초인 경우는 run.php 쪽으로 가서 확인
// 그래프별로 구분: 꺽은선, 막대는 합계로, 파이차트는 통계로 쿼리를 뽑아야 함
// token, mms_idx, dta_group, shf_no, dta_mmi_no, dta_item(minute)
// dta_value_type(sum)
// st_date, en_date
// 디폴트: http://icmms.co.kr/device/json/run.sum.php?token=1099de5drf09&mms_idx=7
// 조건을 바꾸어 보세요.
// http://icmms.co.kr/device/json/run.sum.php?token=1099de5drf09&mms_idx=7&dta_item=daily&st_date=2020-06-01&en_date=2020-06-30
...
//	$list = array('');
    ...
    if(!$en_date) {
        $en1 = sql_fetch("SELECT * FROM {$g5['data_run_sum_table']} {$sql_search}
    ...
    $sql = "SELECT
            ...
            (
                ...
                SELECT
                    ...
                    , dta_shf_no
                    , dta_mmi_no
                FROM {$g5['data_run_sum_table']} AS dta
                {$sql_search}
	...
	for($i=0;$row=sql_fetch_array($rs);$i++){
        ...
        $row['json']['y'] = (int)$row['dta_sum'];   // 측정은 합계값을 넘겨준다.
