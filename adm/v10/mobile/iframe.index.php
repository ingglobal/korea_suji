...
for($i=0;$row=sql_fetch_array($rs);$i++) {
    ...
    <li class="ui-state-default x<?=$column?>" mbd_idx="<?=$row['mbd_idx']?>">
        <span class='graph_icons'>
            ...
            <a href="javascript:" class="chart_setting" style="display:none;"><i class='fa fa-gear'></i></a>
        </span>
        ...
        <script>
            ...
            for($j=0;$j<sizeof($row['data']);$j++) {
                ...
                // in case for http://demo.icmms.co.kr
...
// 그래프 엑셀다운로드
$(document).on('click','.graph_excel_down',function(e){
    e.preventDefault();
    var my_mbd_idx = $(this).closest('li[mbd_idx]').attr('mbd_idx');
    self.location.href = './iframe.index_excel.php?mbd_idx='+my_mbd_idx;
});
...
// parent.postMessage(1800,"<?=G5_URL?>"); // 부모창의 URL 주소
</script>
