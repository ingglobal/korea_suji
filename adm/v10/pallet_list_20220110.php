...
$sql_common = " FROM {$g5['pallet_table']} AS plt
                    LEFT JOIN {$g5['bom_table']} AS bom ON plt.bom_idx = bom.bom_idx
"; 

$where = array();
// 디폴트 검색조건 (used 제외)
$where[] = " plt_status NOT IN ('del','delete','trash') ";
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?php echo $bg; ?>" tr_id="<?php echo $row['orp_idx'] ?>">
        <td class="td_chk">
            <input type="hidden" name="plt_idx[<?php echo $row['plt_idx'] ?>]" value="<?php echo $row['plt_idx'] ?>" class="plt_idx_<?php echo $row['plt_idx'] ?>">
            <label for="chk_<?php echo $i; ?>" class="sound_only"><?php echo get_text($row['plt_idx']); ?></label>
            <input type="checkbox" name="chk[]" value="<?php echo $row['plt_idx'] ?>" id="chk_<?php echo $i ?>">
        ...
        <td class="td_plt_status">
            <select name="plt_status[<?php echo $row['plt_idx'] ?>]" id="plt_status_<?=$row['plt_idx']?>">
                <?=$g5['set_plt_status_value_options']?>
            </select>
            <script>
                $("#plt_status_<?=$row['plt_idx']?>").val('<?=$row['plt_status']?>');
            </script>
        </td><!-- 상태 -->
    </tr>
    <?php
