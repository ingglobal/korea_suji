...
$mms['mms_set_output'] = $mms['mms_set_output'] ?: 'shift';    // 값이 없으면 생산량은 교대 기준
// print_r2($mms);
// $mms['mms_set_output'] = 'date';    // 날짜 기준으로 강제 설정
...
<script>
var seriesOptions = [], graph_type = 'spline', mms_set_output = '<?=$mms['mms_set_output']?>',
    chart, options;
...
$(document).on('click','#fsearch button[type=submit]',function(e){
    ...
    var dta_url = '//icmms.co.kr/device/json/output'+dta_file+'.php?token=1099de5drf09&mms_idx='+mms_idx+'&shf_no='+shf_no+'&dta_type='+dta_type+'&dta_no='+dta_no+'&dta_item='+dta_item+'&dta_unit='+dta_unit+'&st_date='+st_date+'&st_time='+st_time+'&en_date='+en_date+'&en_time='+en_time;
    // console.log(dta_url);
    ...
    Highcharts.getJSON(
        ...
        function(data) {
            ...
            // ===============================================
            // 생산량 누적 (교대기준 or 날짜기준에 따라 다르게 표현됨)
            // ===============================================
            // console.log(data);
            var dta_acc = 0, shift_old, date_old;
            for(i=0;i<data.length;i++) {
                // console.log(data[i]);
                // console.log(data[i].dta_shf_no);
                // console.log(data[i].y);
                // console.log(data[i].dta_date.substring(0,10));
                // 교대가 바뀌면 누적값 초기화
                if(mms_set_output=='shift' && data[i].dta_shf_no != shift_old) {
                    dta_acc = 0;
                }
                // 날짜가 바뀌면 누적값 초기화
                else if(mms_set_output=='date' && data[i].dta_date.substring(0,10) != date_old) {
                    dta_acc = 0;
                }
                // y값 누적
                dta_acc += data[i].y;
                ...
                // 이전 교대값, 날짜 저장
                shift_old = data[i].dta_shf_no;
                date_old = data[i].dta_date.substring(0,10); // 날짜 부분만 저장
            }
...
<div class="btn_fixed_top">
    <a href="./data_output_chart.php" id="btn_add" class="btn btn_02">Chart</a>
    <?php if(!auth_check($auth[$sub_menu],"d",1)) { ?>
    <button class="btn_02 btn btn_insert">일괄입력</button>
    <?php } ?>
    <a href="./data_output_list.php" id="btn_add" class="btn btn_01">목록</a>
</div>
