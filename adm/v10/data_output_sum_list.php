...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $row['mmi'] = sql_fetch(" SELECT mmi_name FROM {$g5['mms_item_table']} WHERE mms_idx = '".$row['mms_idx']."' AND mmi_no = '".$row['dta_mmi_no']."' ");
        ...
        $sql1 = "SELECT SUM(dta_value) AS dta_value_sum
                ...
                WHERE dta_date >= '".$row['dta_date']."'
                    AND dta_shf_no = '".$row['dta_shf_no']."'
                    AND dta_mmi_no = '".$row['dta_mmi_no']."'
                    AND dta_defect = '".$row['dta_defect']."'
                    AND dta_defect_type = '".$row['dta_defect_type']."'
        ";
        // echo $sql1.'<br>';
...
//                    $list[$k1] = substr($row[$k1],0,10);
                    ...
                    $list[$k1] = ($row['dta_defect']) ? $row[$k1].' <span class="font_size_8">'.$row['dta_message'].'</span>' : '';
                }
