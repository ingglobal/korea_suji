...
add_javascript('<script src="https://code.highcharts.com/highcharts.js"></script>',0);
...
<div id="report_wrapper">
	...
	<div class="div_wrapper">
        <div class="div_left">
                ...
                <table class="table01">
                    ...
                    for ($i=0; $row=sql_fetch_array($result); $i++) {
                        ...
                        if($i > 0){//($row['item_name'] != 'total'){
                            $tmp_arr['item_name'] = $g5['mms'][$row['item_name']]['mms_name'];
                            $tmp_arr['target_num'] = $row['target'];
                            $tmp_arr['target_str'] = number_format($row['target']);
                            $tmp_arr['output_total_num'] = $row['output_total'];
                            $tmp_arr['output_total_str'] = number_format($row['output_total']);
                            $tmp_arr['output_good_num'] = $row['output_good'];
                            $tmp_arr['output_good_str'] = number_format($row['output_good']);
                            $tmp_arr['output_defect_num'] = $row['output_defect'];
                            $tmp_arr['output_defect_str'] = number_format($row['output_defect']);
                            array_push($prd_fac_arr,$tmp_arr); 
                        }
                    }
            ...
            #facility_box, #facility_box .highcharts-data-table table {
                min-width: 310px; 
                max-width: 800px;
                margin: 1em auto;
            }

            #facility_box .highcharts-data-table table {
            ...
            #facility_box .highcharts-data-table caption {
                padding: 1em 0;
                font-size: 1.2em;
                color: #555;
            }
            #facility_box .highcharts-data-table th {
                font-weight: 600;
                padding: 0.5em;
            }
            #facility_box .highcharts-data-table td, #facility_box .highcharts-data-table th, #facility_box .highcharts-data-table caption {
                padding: 0.5em;
            }
            #facility_box .highcharts-data-table thead tr, #facility_box .highcharts-data-table tr:nth-child(even) {
                background: #f8f8f8;
            }
            #facility_box .highcharts-data-table tr:hover {
                background: #f1f7ff;
            }
            </style>
            <div id="facility_box">
            ...
            <script>
            var prd_fac = <?=json_encode($prd_fac_arr)?>;
