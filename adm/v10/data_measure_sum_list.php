...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        // dta_value값이 없어서 헷갈림
        $row['dta_value'] = $row['dta_avg'];
        
        // 합산 추출
        $row['st_dt'] = strtotime($row['dta_date'].' 00:00:00');
        $row['en_dt'] = strtotime($row['dta_date'].' 23:59:59');
        $sql1 = "SELECT AVG(dta_value) AS dta_value_sum
                FROM g5_1_data_measure_".$row['mms_idx']."_".$row['dta_type']."_".$row['dta_no']."
                WHERE dta_dt >= '".$row['st_dt']."'
                    AND dta_dt <= '".$row['en_dt']."'
        ";
        // echo $sql1.'<br>';
        $sum1 = sql_fetch($sql1,1);
        $row['dta_value_sum'] = round($sum1['dta_value_sum'],2);
        
        // 조정 버튼 (합산값이랑 다를 때만 조정버튼)
        if($row['dta_value_sum'] != $row['dta_avg'])
...
//                    $list[$k1] = substr($row[$k1],0,10);
                    ...
                    // echo $row['dta_value'].' =? '.$row[$k1].'<br>'; // 위에서 값이 이미 바뀌어서 바로 비교하면 안 됨
                    ...
                    $list[$k1] = $row[$k1].' <span class="font_size_8">'.$row['mta']['dta_type_label-'.$row['dta_type'].'-'.$row['dta_no']].'</span>';
                }
                else if($k1=='dta_type') {
