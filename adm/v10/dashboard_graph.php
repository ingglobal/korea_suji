...
#title_com_name {margin-left:10px;font-size:0.8em;color:#818181;}
</style>
...
<!-- 상단에 업체명 추가 -->
<span id="title_com_name"><?=$com['com_name']?></span>
...
<input type="hidden" name="dta_minsec" value="<?=$dta_minsec?>" id="dta_minsec" class="frm_input" style="width:20px;">
    ...
    <a href="./data_measure_add.php?file_name=<?php echo $g5['file_name']?>" class="btn btn_03 btn_add_chart">추가</a>
...
<script>
var seriesOptions = [], navigator_type = 'spline',
    chart, options;

// 그래프 함수
function createChart() {
    // var graph_type = $('select[name=graph_type]').val() || '<?=$graph_type?>'; // 그래프 종류
    // var chart = new Highcharts.stockChart({
    options = {
        chart: {
            renderTo: 'chart1',
//            type: graph_type,   // 전체타입설정 line, spline, area, areaspline, column, bar, pie, scatter, gauge, arearange, areasplinerange, columnrange
            events: {
                redraw: function() {
...
$(document).on('click','button[type=submit], #div_shift a, #div_code a',function(e){
    ...
    // var graph_type = frm.find('select[name=graph_type]').val() || '<?=$graph_type?>';
    ...
    var dta_url = '//icmms.co.kr/device/json/error'+dta_file+'.php?token=1099de5drf09&mms_idx='+mms_idx+'&dta_group='+dta_group+'&shf_no='+shf_no+'&dta_code='+dta_code+'&dta_item='+dta_item+'&dta_unit='+dta_unit+'&st_date='+st_date+'&st_time='+st_time+'&en_date='+en_date+'&en_time='+en_time;
    dta_loading('show');
    ...
    Highcharts.getJSON( dta_url,
        function(data) {
            ...
            if( tag_name == 'BUTTON' ) {
                // 교대수를 불러와서 표시
                var dta_shift_url = '//icmms.co.kr/device/json/error.code.php?token=1099de5drf09&mms_idx='+mms_idx+'&dta_group='+dta_group+'&dta_groupby=shf_no&st_date='+st_date_new+'&st_time='+st_time_new+'&en_date='+en_date_new+'&en_time='+en_time_new;
                ...
                $.getJSON(dta_shift_url,{"aj":"list"},function(res) {
                    ...
                    try{
                        $.each(res,function(i,v){
                            ...
                            dta_shift_dom += '<a href="javascript:" shf_no="'+v['shf_no']+'"><b>'+v['shf_no']+'교대</b>: <span class="dta_count">'+thousand_comma(v['dta_count'])+'</span>회</a> ';
                        });
                        // 교대수가 한개만 있으면 숨김
                        if(res.length<=1)
                            $('#div_shift').hide();
                ...
                // 코드를 불러와서 표시
                var dta_code_url = '//icmms.co.kr/device/json/error.code.php?token=1099de5drf09&mms_idx='+mms_idx+'&dta_group='+dta_group+'&dta_groupby=dta_code&st_date='+st_date_new+'&st_time='+st_time_new+'&en_date='+en_date_new+'&en_time='+en_time_new;
                ...
                $.getJSON(dta_code_url,{"aj":"list"},function(res) {
                    ...
                    try{
                        $.each(res,function(i,v){
                            ...
                            dta_code_dom += '<li><a href="javascript:" dta_code="'+v['dta_code']+'"><b>'+v['dta_code']+'</b>: <span class="dta_count">'+thousand_comma(v['dta_count'])+'</span>회</a> (<span>'+cut_str(v['dta_message'],18)+'</span>)</li>';
                        });
                        // 없으면 숨김
                        if(res.length<=0)
                            $('#div_code').hide();
...
$('.btn_bigger, .btn_orig, .btn_smaller').click(function(e) {
    ...
    chart = new Highcharts.stockChart(options); // 옵션 변경 후 그래프 재생성 (옵션 초기화 아님!)
    removeLogo();
...
$(function(e) {
    ...
    // 상단에 업체명 추가
    $('#container_title').append( $('#title_com_name') );

});
</script>

<div class="btn_fixed_top">
    <a href="./dashboard_graph.php" id="btn_add" class="btn btn_02">정주기</a>
    <a href="./dashboard_graph_real.php" id="btn_add" class="btn btn_02">비주기측정</a>
</div>
